...
import { RaceMiniMap } from "./components/RaceMiniMap";
...
const initialTelemetry: GameTelemetry = { speedKph: 0, rpm: 5000, gear: "N", lap: 1, laps: 3, position: VEHICLES.length, racers: VEHICLES.length, currentLapMs: 0, totalTimeMs: 0, fuel: 100, tires: [100, 100, 100, 100], boost: 40, drs: false, drifting: false, wrongWay: false, timeGap: 0, defensiveWake: false, lateralOffset: 0, nitroActive: false, miniBoostActive: false, speedIntensity: 0, racePhase: "countdown", collisionPulse: 0, aiSpeedSpreadKph: 0, aiFieldSpreadMeters: 0, raceMapRacers: [] };
...
function publicAsset(path: string): string { return `${import.meta.env.BASE_URL}${path.replace(/^\//, "")}`; }
...
function Home({ player, navigate }: { player: PlayerStateV1; navigate: (screen: Screen) => void }) {
  ...
  return <main className="home page">
    ...
    <section className="dashboard-grid">
      ...
      <button className="feature-card" onClick={() => navigate("multiplayer")}><span className="status-dot online"/><div><small>LIVE PADDOCK</small><h2>多人房間</h2><p>WebSocket 即時連線 · 最多 10 人</p></div><b>連線 →</b></button>
...
interface MultiplayerUi { status: NetworkStatus; detail: string; roomCode: string; hostId: string; trackId: TrackId; players: RoomPlayer[]; completed: boolean; classification: RaceClassification[]; }
function Multiplayer({ ui, playerId, player, onCreate, onJoin, onReady, onRetry }: { ui: MultiplayerUi; playerId: string; player: PlayerStateV1; onCreate: (name: string, trackId: TrackId) => void; onJoin: (name: string, code: string) => void; onReady: (ready: boolean) => void; onRetry: () => void }) {
  ...
  const inRoom = !!ui.roomCode && ["online", "reconnecting"].includes(ui.status); const self = ui.players.find((entry) => entry.id === playerId);
  const remember = () => localStorage.setItem("f1-kart.player-name", name);
  const busy = ui.status === "connecting" || ui.status === "reconnecting";
  return <main className="page multiplayer-page"><div className="page-heading"><div><small>LIVE PADDOCK</small><h1>多人賽事大廳</h1></div><div className={`connection ${ui.status}`}><i/>{ui.status === "online" ? "WEBSOCKET ONLINE" : ui.status === "reconnecting" ? "RECONNECTING" : ui.status.toUpperCase()}</div></div>
    {!inRoom && <div className="lobby-layout"><section className="lobby-panel"><div className="panel-number">01</div><h2>車手身份</h2><label>玩家名稱<input maxLength={16} value={name} onChange={(event) => setName(event.target.value)} /></label><div className="lobby-car"><CarShowcase compact vehicleId={player.selectedVehicleId} livery={player.selectedLiveries[player.selectedVehicleId] ?? 0}/><span>出賽車輛<b>{VEHICLE_BY_ID[player.selectedVehicleId].name}</b></span></div></section><section className="lobby-panel"><div className="panel-number">02</div><h2>建立房間</h2><div className="track-toggle">{TRACKS.map((track) => <button key={track.id} className={track.id === trackId ? "active" : ""} onClick={() => setTrackId(track.id)}>{track.shortName}<small>{track.laps} 圈</small></button>)}</div><button className="primary wide" disabled={name.trim().length < 2 || busy} onClick={() => { remember(); onCreate(name.trim(), trackId); }}>建立私人房間</button><div className="or"><span>或使用代碼加入</span></div><div className="join-row"><input maxLength={6} placeholder="ROOM CODE" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())}/><button disabled={code.length !== 6 || name.trim().length < 2 || busy} onClick={() => { remember(); onJoin(name.trim(), code); }}>加入</button></div>{ui.status === "error" && <div className="network-error"><p>{ui.detail}</p><button className="ghost" onClick={onRetry}>重新連線</button></div>}</section></div>}
    {inRoom && <section className="room-panel"><div className="room-code"><small>ROOM CODE</small><b>{ui.roomCode}</b><button onClick={() => void navigator.clipboard?.writeText(ui.roomCode)}>複製代碼</button></div><div className="room-track"><span>{TRACK_BY_ID[ui.trackId].shortName}</span><div><b>{TRACK_BY_ID[ui.trackId].name}</b><small>{TRACK_BY_ID[ui.trackId].laps} 圈 · 最多 10 人</small></div></div><div className="player-grid">{ui.players.map((entry, index) => <article key={entry.id} className={`${entry.ready ? "ready" : ""} ${entry.connected ? "" : "disconnected"}`}><span>{String(index+1).padStart(2,"0")}</span><div><b>{entry.name}{entry.id === ui.hostId ? "　♛" : ""}</b><small>{VEHICLE_BY_ID[entry.vehicleId].name}</small></div><i>{entry.dnf ? "DNF" : !entry.connected ? "RECONNECTING" : entry.ready ? "READY" : "WAITING"}</i></article>)}{Array.from({length: Math.max(0,10-ui.players.length)},(_,index)=><article className="empty" key={index}><span>--</span><div><b>等待車手</b><small>空車位</small></div></article>)}</div><div className="ready-bar"><p>{ui.detail || (ui.players.length < 2 ? "至少需要 2 名玩家" : "所有玩家準備後自動開始倒數")}</p><button disabled={ui.status !== "online" || ui.completed} className={self?.ready ? "ghost" : "primary"} onClick={() => onReady(!self?.ready)}>{self?.ready ? "取消準備" : "準備出賽"}</button></div></section>}
  </main>;
}
...
function RaceHud({ telemetry, config }: { telemetry: GameTelemetry; config: RaceConfig }) {
  ...
  return <div className={`race-hud ${telemetry.nitroActive ? "nitro-active" : telemetry.miniBoostActive ? "mini-active" : ""} ${telemetry.drifting ? "is-drifting" : ""} ${telemetry.speedIntensity > 0.2 ? "is-high-speed" : ""} ${telemetry.collisionPulse > 0.08 ? "has-impact" : ""}`} style={{"--speed-intensity":telemetry.speedIntensity,"--collision-pulse":telemetry.collisionPulse} as React.CSSProperties} data-lateral-offset={telemetry.lateralOffset.toFixed(3)} data-nitro-active={String(telemetry.nitroActive)} data-mini-boost-active={String(telemetry.miniBoostActive)} data-race-phase={telemetry.racePhase} data-speed-intensity={telemetry.speedIntensity.toFixed(3)} data-ai-speed-spread={telemetry.aiSpeedSpreadKph.toFixed(2)} data-ai-field-spread={telemetry.aiFieldSpreadMeters.toFixed(2)}><div className="speed-vignette"/>{(telemetry.nitroActive||telemetry.miniBoostActive)&&<div className={`boost-effects ${telemetry.nitroActive?"nitro":"mini"}`}><span>{telemetry.nitroActive?"NITRO":"MINI TURBO"}</span></div>}{telemetry.racePhase === "coasting" && <div className="finish-coast-banner"><small>CHEQUERED FLAG</small><b>FINISH</b><span>動力已切斷 · 車輛自動滑行</span></div>}<div className="hud-top"><div className="position-box"><small>POSITION</small><b>{telemetry.position}<em>/{telemetry.racers}</em></b></div><div className="race-info"><small>{TRACK_BY_ID[config.trackId].shortName} GRAND PRIX</small><b>LAP {telemetry.lap}<em> / {telemetry.laps}</em></b></div><div className="weather-box"><span>{WEATHER_ICONS[config.weather]}</span><div><small>TRACK STATUS</small><b>{WEATHER_LABELS[config.weather]}</b></div></div></div>
    ...
    <RaceMiniMap telemetry={telemetry} config={config}/>
...
function GameView({ config, player, quality, volume, paused, networkSuspended, remoteSnapshots, onPause, onCountdown, onTelemetry, onFinish, onNetworkSnapshot }: { config: RaceConfig; player: PlayerStateV1; quality: Quality; volume: number; paused: boolean; networkSuspended: boolean; remoteSnapshots: RaceSnapshot[]; onPause:(paused:boolean)=>void; onCountdown:(value:string|null)=>void; onTelemetry:(t:GameTelemetry)=>void; onFinish:(r:RaceResult)=>void; onNetworkSnapshot:(snapshot:ClientRaceSnapshot)=>void }) {
  ...
  useEffect(() => engine.current?.setPaused(paused), [paused]); useEffect(() => engine.current?.setNetworkSuspended(networkSuspended), [networkSuspended]); useEffect(() => engine.current?.setRemoteSnapshots(remoteSnapshots), [remoteSnapshots]);
  return <div className="game-canvas" ref={host}/>;
}

function Results({ result, achievements, multiplayerState, isHost, onRematch, back }: { result: RaceResult; achievements: AchievementDefinition[]; multiplayerState?: "waiting" | "ready"; isHost?: boolean; onRematch?: () => void; back: () => void }) {
  return <main className="results-page"><div className="results-backdrop"/><section className="results-panel"><div className="result-rank"><small>OFFICIAL CLASSIFICATION</small><span>P{result.position}</span><h1>{result.position === 1 ? "VICTORY" : result.position <= 3 ? "PODIUM FINISH" : "RACE COMPLETE"}</h1><p>{TRACK_BY_ID[result.config.trackId].name} · {WEATHER_LABELS[result.config.weather]}</p></div><div className="result-columns"><div><h2>圈速成績</h2>{result.laps.map((lap)=><div className="lap-result" key={lap.lap}><span>LAP {String(lap.lap).padStart(2,"0")}</span><b>{formatTime(lap.timeMs)}</b>{lap.personalBest && <i>PB</i>}</div>)}<div className="lap-result total"><span>伺服器總時間</span><b>{formatTime(result.totalTimeMs)}</b></div></div><div><h2>賽事獎勵</h2><div className="reward-total"><span>◉</span><b>+{result.rewards.awardedCoins}</b><small>金幣</small></div><div className="reward-xp">+{result.rewards.xp} XP</div>{result.rewards.labels.map((label)=><p className="bonus" key={label}>◆　{label}</p>)}{result.rewards.capped && <p className="cap-warning">今日金幣已達上限</p>}</div></div>{achievements.length>0&&<div className="new-achievements"><small>ACHIEVEMENT UNLOCKED</small>{achievements.map((achievement)=><span key={achievement.id}>◆　{achievement.name}</span>)}</div>}{multiplayerState === "ready" && isHost && <button className="primary xl wide" onClick={onRematch}>再來一場</button>}{multiplayerState === "ready" && !isHost && <p className="rematch-waiting">等待房主發起重賽…</p>}{multiplayerState === "waiting" && <p className="rematch-waiting">等待其他車手完賽…</p>}<button className={multiplayerState ? "ghost wide" : "primary xl wide"} onClick={back}>{multiplayerState ? "離開房間" : "返回主畫面"}</button></section></main>;
}
...
export default function App() {
  ...
  const [mp, setMp] = useState<MultiplayerUi>({status:"idle",detail:"",roomCode:"",hostId:"",trackId:"velocity",players:[],completed:false,classification:[]});
  const mpStateRef = useRef(mp); mpStateRef.current = mp;
  const screenRef = useRef(screen); screenRef.current = screen;
  const pendingMultiplayerResult = useRef<RaceResult|null>(null);
  const commitResultRef = useRef<(raceResult: RaceResult) => void>(() => undefined);
  ...
  if (!multiplayer.current) multiplayer.current = new MultiplayerClient({
    ...
    onRoom:(roomCode,hostId,trackId,players)=>{setMp((value)=>({...value,roomCode,hostId,trackId,players}));if(screenRef.current==="home")setScreen("multiplayer");},
    onCountdown:(startsAt,seed,weather,serverTimeOffsetMs)=>{const room=mpStateRef.current;const gridPosition=Math.max(0,room.players.findIndex((entry)=>entry.id===multiplayer.current?.playerId));const config:RaceConfig={trackId:room.trackId,lapCount:TRACK_BY_ID[room.trackId].laps,difficulty:"normal",weather,seed,mode:"multiplayer",roomCode:room.roomCode,gridPosition,startsAt,serverTimeOffsetMs};setTelemetry({...initialTelemetry,laps:config.lapCount,position:gridPosition+1,racers:room.players.length});setRaceConfig(config);setPaused(false);setRemoteSnapshots([]);setMp((value)=>({...value,detail:`${WEATHER_LABELS[weather]} · 同步倒數中`}));setScreen("race");},
    onRaceStart:()=>setMp((value)=>({...value,detail:"賽事進行中"})),
    onSnapshots:setRemoteSnapshots,
    onRaceResult:(serverResult:Extract<ServerMessage,{type:"race_result"}>)=>{if(serverResult.playerId!==multiplayer.current?.playerId)return;const pending=pendingMultiplayerResult.current;if(!pending)return;const today=new Date().toLocaleDateString("en-CA");const daily=playerRef.current.dailyEarnings.date===today?playerRef.current.dailyEarnings.coins:0;const rewards=calculateRewards(serverResult.rank,pending.config,false,playerRef.current.winStreak,daily);pendingMultiplayerResult.current=null;commitResultRef.current({...pending,position:serverResult.rank,totalRacers:serverResult.totalRacers,totalTimeMs:serverResult.totalTimeMs,fastestLap:false,rewards});},
    onRaceComplete:(classification)=>{setMp((value)=>({...value,completed:true,classification,detail:"賽事已結束"}));const own=classification.find((entry)=>entry.playerId===multiplayer.current?.playerId);if(screenRef.current==="race"&&own?.status==="dnf"){pendingMultiplayerResult.current=null;setToast("賽事已結束，你的成績為 DNF");setScreen("multiplayer");}},
    onRoomReset:()=>{pendingMultiplayerResult.current=null;setResult(null);setRemoteSnapshots([]);setMp((value)=>({...value,completed:false,classification:[],detail:"房間已重設，請重新準備"}));setScreen("multiplayer");}
  });
  useEffect(() => { multiplayer.current?.start(); return () => multiplayer.current?.dispose(); }, []);
  ...
  const commitRaceResult = (raceResult:RaceResult) => {const draft=clonePlayer(player);const today=new Date().toLocaleDateString("en-CA");if(draft.dailyEarnings.date!==today)draft.dailyEarnings={date:today,coins:0};draft.coins+=raceResult.rewards.awardedCoins;draft.dailyEarnings.coins+=raceResult.rewards.awardedCoins;draft.stats.totalCoinsEarned+=raceResult.rewards.awardedCoins;applyXp(draft,raceResult.rewards.xp);if(raceResult.config.mode!=="mock"){draft.stats.races+=1;draft.stats.laps+=raceResult.config.lapCount;draft.stats.collisions+=raceResult.telemetry.collisions;draft.stats.driftSeconds+=raceResult.telemetry.totalDriftSeconds;if(!draft.stats.weatherFinished.includes(raceResult.config.weather))draft.stats.weatherFinished.push(raceResult.config.weather);if(raceResult.position<=3)draft.stats.podiums+=1;if(raceResult.position===1){draft.stats.wins+=1;draft.winStreak+=1;if(!draft.stats.trackWins.includes(raceResult.config.trackId))draft.stats.trackWins.push(raceResult.config.trackId);}else draft.winStreak=0;}const best=Math.min(...raceResult.laps.map((lap)=>lap.timeMs));if(Number.isFinite(best))draft.bestLaps[raceResult.config.trackId]=Math.min(best,draft.bestLaps[raceResult.config.trackId]??Infinity);const unlocked=evaluateAchievements(draft,raceResult);unlock(draft,unlocked);savePlayer(draft);setPlayer(draft);setNewAchievements(unlocked);setResult(raceResult);setScreen("results");};
  commitResultRef.current=commitRaceResult;
  const finishRace = (raceResult:RaceResult) => {if(raceResult.config.mode==="multiplayer"){pendingMultiplayerResult.current=raceResult;multiplayer.current?.finish();setToast("等待伺服器確認正式成績…");return;}commitRaceResult(raceResult);};
  const leaveMultiplayer=()=>{multiplayer.current?.disconnect();pendingMultiplayerResult.current=null;setMp({status:"idle",detail:"",roomCode:"",hostId:"",trackId:"velocity",players:[],completed:false,classification:[]});setResult(null);setScreen("home");};
  const navigate=(next:Screen)=>{if(screen==="race")return;if(screen==="multiplayer"&&next!=="multiplayer")multiplayer.current?.disconnect();setScreen(next);};
  ...
  const networkSuspended=!!raceConfig&&raceConfig.mode==="multiplayer"&&mp.status!=="online";
  ...
  return <div className="app-shell"><TopBar player={player} screen={screen} navigate={navigate} quality={settings.quality} setQuality={changeQuality}/>
    ...
    {screen==="multiplayer"&&<Multiplayer ui={mp} playerId={multiplayer.current.playerId} player={player} onCreate={(name,trackId)=>multiplayer.current?.connect({type:"create",name,vehicleId:player.selectedVehicleId,trackId})} onJoin={(name,roomCode)=>multiplayer.current?.connect({type:"join",name,vehicleId:player.selectedVehicleId,roomCode})} onReady={(ready)=>multiplayer.current?.setReady(ready)} onRetry={()=>multiplayer.current?.retry()}/>}
    {screen==="race"&&raceConfig&&<div className="race-screen"><GameView config={raceConfig} player={player} quality={settings.quality} volume={settings.volume} paused={paused} networkSuspended={networkSuspended} remoteSnapshots={remoteSnapshots} onPause={setPaused} onCountdown={setCountdown} onTelemetry={setTelemetry} onFinish={finishRace} onNetworkSnapshot={(snapshot)=>multiplayer.current?.sendSnapshot(snapshot)}/><RaceHud telemetry={telemetry} config={raceConfig}/>{countdown&&<div className={`countdown ${countdown==="GO"?"go":""}`}><small>{WEATHER_ICONS[raceConfig.weather]} {WEATHER_LABELS[raceConfig.weather]} · {TRACK_BY_ID[raceConfig.trackId].shortName}</small><b>{countdown}</b></div>}{networkSuspended&&<div className="pause-overlay"><section><small>NETWORK CONTROL</small><h1>{mp.status==="reconnecting"?"重新連線中":"連線已中斷"}</h1><p>{mp.detail}</p>{mp.status==="error"&&<button className="ghost wide" onClick={()=>setScreen("multiplayer")}>返回多人賽事</button>}</section></div>}{paused&&<div className="pause-overlay"><section><small>RACE CONTROL</small><h1>比賽暫停</h1><button className="primary wide" onClick={()=>setPaused(false)}>繼續比賽</button><button className="ghost wide" onClick={()=>{setPaused(false);setScreen("home");}}>退出至主畫面</button><p>按 ESC 或 P 也可繼續</p></section></div>}</div>}
    {screen==="results"&&result&&<Results result={result} achievements={newAchievements} multiplayerState={result.config.mode==="multiplayer"?(mp.completed?"ready":"waiting"):undefined} isHost={mp.hostId===multiplayer.current.playerId} onRematch={()=>multiplayer.current?.rematch()} back={result.config.mode==="multiplayer"?leaveMultiplayer:()=>{setResult(null);setScreen("home");}}/>}
